...
class EvalCallback():
    def __init__(self, net, input_shape, num_classes, image_ids, dataset_path, log_dir, cuda, \
                 miou_out_path=".temp_miou_out", eval_flag=True, period=1, img_extension=".tif"):
    ...
    def on_epoch_end(self, epoch, model_eval):
        if epoch % self.period == 0 and self.eval_flag:
            ...
            for image_id in tqdm(self.image_ids):
                ...
                #   从文件中读取图像
                # -------------------------------#
                image_path = os.path.join(self.dataset_path, f"JPEGImages/{image_id}{self.img_extension}")
                image = Image.open(image_path)
                image = Image.fromarray(image) if self.img_extension == ".tif" else image
                # ------------------------------#
                #   获得预测txt
                # ------------------------------#
