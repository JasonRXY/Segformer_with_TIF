...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes=None, img_extension=".jpg"):
    print('Num classes', num_classes)
    hist = np.zeros((num_classes, num_classes))

    gt_imgs = [join(gt_dir, x + img_extension) for x in png_name_list]
    pred_imgs = [join(pred_dir, x + img_extension) for x in png_name_list]

    for ind in range(len(gt_imgs)):
        if img_extension == ".tif":
            pred = np.array(tiff.imread(pred_imgs[ind]))
            label = np.array(tiff.imread(gt_imgs[ind]))
        else:
    ...
    return np.array(hist, int), IoUs, PA_Recall, Precision  # 将 np.int 替换为 int
